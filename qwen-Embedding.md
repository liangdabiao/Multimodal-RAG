...
您需要已[获取API Key](/help/zh/model-studio/get-api-key)并[配置API Key到环境变量](/help/zh/model-studio/configure-api-key-through-environment-variables)。如果通过OpenAI SDK或DashScope SDK进行调用，还需要[安装SDK](/help/zh/model-studio/install-sdk)。
...
# 如果使用华北2（北京）地域的模型，需要将base_url替换为：https://dashscope.aliyuncs.com/api/v1
...
public class Main {
    static {
        ...
        // 华北2（北京）地域需要替换为: https://dashscope.aliyuncs.com/api/v1
    }
...
```
# ======= 重要提示 =======
# 如果使用华北2（北京）地域的模型，需要将base_url替换为：https://dashscope.aliyuncs.com/api/v1/services/embeddings/text-embedding/text-embedding
# === 执行时请删除该注释 ====
    ...
    public static void main(String[] args) {
        try {
            ...
            MultiModalEmbeddingParam param = MultiModalEmbeddingParam.builder()
                // 若没有配置环境变量，请用百炼API Key向下方添加：.apiKey("sk-xxx")
                .model("tongyi-embedding-vision-plus")
...
-   **处理多模态内容**：
    ...
    -   **融合向量：**若要将单模态或混合模态输入表征为融合向量，适用于跨模态检索、图搜等场景，可使用 `qwen2.5-vl-embedding`、`qwen3-vl-embedding`。例如，输入一张衬衫图片并附加文本”找相似风格但更显年轻的款式”，模型能将图像和文本指令融合成一个向量进行理解。
...
-   **处理大规模数据**：若您需要处理大规模、非实时的文本数据，建议使用 `text-embedding-v4` 并结合 [OpenAI兼容-Batch调用](/help/zh/model-studio/batch-interfaces-compatible-with-openai/)，以显著降低成本。
...
| **模型名称** | **向量维度** | **批次大小** | **单批次最大处理Token数（**[注](/help/zh/model-studio/billing-for-model-studio#f300d75bd5rb2)**）** | **单价（每百万输入Token）** | **支持语种** | **免费额度**[（注）](/help/zh/model-studio/new-free-quota#591f3dfedfyzj) |
| --- | --- | --- | --- | --- | --- | --- |
| text-embedding-v4 > 属于[Qwen3-Embedding](https://qwenlm.github.io/zh/blog/qwen3-embedding/)系列 | 2,048、1,536、1,024（默认）、768、512、256、128、64 | 10  | 8,192 | $0.07 | 中文、英语、西班牙语、法语、葡萄牙语、印尼语、日语、韩语、德语、俄罗斯语等100+主流语种 | 100万Token 有效期：百炼开通后90天内 |
| text-embedding-v3 | 1,024（默认）、768、512 | 中文、英语、西班牙语、法语、葡萄牙语、印尼语、日语、韩语、德语、俄罗斯语等50+主流语种 | 50万Token 有效期：百炼开通后90天内 |

## 北京

| **模型名称** | **向量维度** | **批次大小** | **单批次最大Token数（**[注](/help/zh/model-studio/billing-for-model-studio#f300d75bd5rb2)**）** | **单价（每百万输入Token）** | **支持语种** |
| --- | --- | --- | --- | --- | --- |
| text-embedding-v4 > 属于[Qwen3-Embedding](https://qwenlm.github.io/zh/blog/qwen3-embedding/)系列 | 2,048、1,536、1,024（默认）、768、512、256、128、64 | 10  | 8,192 | $0.072 | 中文、英语、西班牙语、法语、葡萄牙语、印尼语、日语、韩语、德语、俄罗斯语等100+主流语种及多种编程 |

## 中国香港

| **模型名称** | **向量维度** | **批次大小** | **单批次最大Token数（**[注](/help/zh/model-studio/billing-for-model-studio#f300d75bd5rb2)**）** | **单价（每百万输入Token）** | **支持语种** |
| --- | --- | --- | --- | --- | --- |
| text-embedding-v4 > 属于[Qwen3-Embedding](https://qwenlm.github.io/zh/blog/qwen3-embedding/)系列 | 2,048、1,536、1,024（默认）、768、512、256、128、64 | 10  | 8,192 | $0.07 | 中文、英语、西班牙语、法语、葡萄牙语、印尼语、日语、韩语、德语、俄罗斯语等100+主流语种及多种编程 |
...
| **模型名称** | **向量维度** | **文本长度限制** | **图片大小限制** | **视频大小限制** | **单价（每百万输入Token）** | **免费额度**[（注）](/help/zh/model-studio/new-free-quota#591f3dfedfyzj) |
| --- | --- | --- | --- | --- | --- | --- |
| tongyi-embedding-vision-plus | 1152 | 1,024 Token | 单张大小不超过**3 MB**。支持多图，最多支持输入**8张** | 视频文件大小不超过 **10 MB** | 图片/视频：$0.09 文本：$0.09 | 100万Token 有效期：百炼开通后90天内 |
| tongyi-embedding-vision-flash | 768 | 图片/视频：$0.03 文本：$0.09 |

## 北京

| **模型名称** | **向量维度** | **文本长度限制** | **图片大小限制** | **视频大小限制** | **单价（每百万输入Token）** |
| --- | --- | --- | --- | --- | --- |
| qwen3-vl-embedding | 2560（默认）, 2048, 1536, 1024, 768, 512, 256 | 32,000 Token | 最多 **1 张**且单张大小不超过**5 MB** | 视频文件大小不超过 **50 MB** | 图片/视频：$0.258 文本：$0.1 |
| multimodal-embedding-v1 | 1024 | 512 Token | 最多 **8 张**且单张大小不超过**3 MB** | 视频文件大小不超过 **10 MB** | 免费试用 |
...
| --- | --- | --- | --- | --- |
| **模型** | **文本** | **图片** | **视频** | **单次请求条数** |
| qwen3-vl-embedding | 支持中、英、日、韩、法、德等33种主流语言 | JPEG, PNG, WEBP, BMP, TIFF, ICO, DIB, ICNS, SGI（支持URL或Base64） | MP4, AVI, MOV（仅支持URL） | 一次请求中传入内容元素总数不超过 20；图片数量不超过5；图片、文本、视频共享该上限。 |
| **多模态独立向量模型** |   |   |     |   |
| **模型** | **文本** | **图片** | **视频** | **单次请求条数** |
| tongyi-embedding-vision-plus | 中/英文 | JPG, PNG, BMP (支持URL或Base64) | MP4, MPEG, AVI, MOV, MPG, WEBM, FLV, MKV（仅支持URL） | 暂无传入内容元素数量限制，输入内容总Token数不超过Token数量上限即可。 |
...
`text-embedding-v4` 、 `text-embedding-v3`、`tongyi-embedding-vision-plus`、`tongyi-embedding-vision-flash`、`qwen3-vl-embedding`支持自定义向量维度。更高的维度能保留更丰富的语义信息，但也会相应增加存储和计算成本。
...
-   ### **通用文本向量**
    ...
    -   [同步处理模型接口API详情](/help/zh/model-studio/text-embedding-synchronous-api)
        
-   ### **多模态向量**
    
    [多模态向量模型接口API详情](/help/zh/model-studio/multimodal-embedding-api-reference)
...
关于模型的限流条件，请参见[限流](/help/zh/model-studio/rate-limit)。
