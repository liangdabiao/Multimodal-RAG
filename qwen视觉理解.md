...
**在线体验：**访问[阿里云百炼控制台](https://modelstudio.console.alibabacloud.com/)，在页面右上角选择目标地域，进入[视觉模型](https://modelstudio.console.alibabacloud.com/ap-southeast-1/?tab=dashboard#/efm/model_experience_center/vision)页面进行体验。

## **快速开始**

**前提条件**

-   已[获取 API Key](/help/zh/model-studio/get-api-key)并[配置API Key到环境变量](/help/zh/model-studio/configure-api-key-through-environment-variables)。
    
-   如果通过 SDK 进行调用，需安装[SDK](/help/zh/model-studio/install-sdk)，其中 DashScope Python SDK 版本不低于1.24.6，DashScope Java SDK 版本不低于 2.21.10。
...
completion = client.chat.completions.create(
    model="qwen3.6-plus",  # 此处以qwen3.6-plus为例，可按需更换模型名称。模型列表：https://www.alibabacloud.com/help/model-studio/getting-started/models
    messages=[
        {
            "role": "user",
            "content": [
...
模型的名称、上下文、价格、快照版本等信息请参见百炼控制台；并发限流条件请参考[限流](/help/zh/model-studio/rate-limit)。

**模型特性对比**

| **模型** | [**深度思考**](/help/zh/model-studio/visual-reasoning) | [**工具调用**](/help/zh/model-studio/qwen-function-calling) | [上下文缓存](/help/zh/model-studio/context-cache) | [结构化输出](/help/zh/model-studio/qwen-structured-output) | **识别的语言种类** |
| --- | --- | --- | --- | --- | --- |
...
response = dashscope.MultiModalConversation.call(
    ...
    # 若没有配置环境变量，请用百炼API Key将下行替换为：api_key="sk-xxx"
    api_key=os.getenv('DASHSCOPE_API_KEY'),
    model='qwen3.6-plus', #  此处以qwen3.6-plus为例，可按需更换模型名称。模型列表：https://www.alibabacloud.com/help/zh/model-studio/getting-started/models
    messages=messages
)
...
public class Main {
   static {
    ...
    public static void simpleMultiModalConversationCall()
            throws ApiException, NoApiKeyException, UploadFileException {
        MultiModalConversation conv = new MultiModalConversation();
        // fps参数控制视频抽帧频率，表示每隔 1/fps 秒抽取一帧，完整用法请参见：https://www.alibabacloud.com/help/zh/model-studio/use-qwen-by-calling-api?#2ed5ee7377fum
        ...
        MultiModalConversationParam param = MultiModalConversationParam.builder()
                // 如果使用北京地域的模型，需要使用北京地域的 API Key，获取链接：https://www.alibabacloud.com/help/zh/model-studio/get-api-key
                // 若没有配置环境变量，请用百炼API Key将下行替换为：.apiKey("sk-xxx")
                .apiKey(System.getenv("DASHSCOPE_API_KEY"))
                .model("qwen3.6-plus")
                .messages(Arrays.asList(userMessage))
...
completion = client.chat.completions.create(
    model="qwen3.6-plus", # 此处以qwen3.6-plus为例，可按需更换模型名称。模型列表：https://www.alibabacloud.com/help/zh/model-studio/models
...
public class Main {
    ...
    private static final String MODEL_NAME = "qwen3.6-plus";  // 此处以qwen3.6-plus为例，可按需更换模型名称。模型列表：https://www.alibabacloud.com/help/model-studio/getting-started/models
    ...
    messages=[
        {
            ...
            "content": [
                {
                    "type": "image_url",
                    # 传入Base64图像数据， 需要注意，图像格式（即image/{format}）需要与支持的图片列表中的Content Type保持一致。"f"是字符串格式化的方法。
...
| `qwen-vl-max`、`qwen-vl-max-latest`、`qwen-vl-max-2025-08-13`、`qwen-vl-plus`、`qwen-vl-plus-latest`、`qwen-vl-plus-2025-08-15`模型 | `32*32` | `true` | `max_pixels` 无效 | `16384 Token` | `16777216`（即`16384*32*32`） |
...
-   [多轮对话](/help/zh/model-studio/multi-round-conversation#6feb3eb136g3q)
    
-   [流式输出](/help/zh/model-studio/stream#39de325514ak9)
...
-   **公网URL**：提供一个公网可访问的文件地址，支持 HTTP 或 HTTPS 协议。为获得最佳稳定性和性能，可将文件[上传至OSS](/help/zh/oss/user-guide/console-quick-start)，获取公网 URL。
...
-   **图像/视频预处理：**视觉理解模型对输入的文件有大小限制，如需压缩文件请参见[图像或视频压缩方法](/help/zh/model-studio/vision#ec8e0a8e03moe)。
...
-   **处理文本文件：**视觉理解模型仅支持处理图像格式的文件，无法直接处理文本文件。但可使用以下替代方案：
    ...
    -   [Qwen-Long](/help/zh/model-studio/long-context-qwen-long)支持处理文本文件，可用于解析文件内容。
        

-   **容错与稳定性**
    
    -   超时处理：在非流式调用中，180 秒内模型没有结束输出通常会触发超时报错。为了提升用户体验，超时后响应体中会将已生成的内容返回。如果响应头包含`x-dashscope-partialresponse：true`，表示本次响应触发了超时。您可以使用[前缀续写](/help/zh/model-studio/partial-mode)功能（支持部分模型），将已生成的内容添加到 messages 数组并再次发出请求，使大模型继续生成内容。详情请参见：[基于不完整输出进行续写](/help/zh/model-studio/partial-mode#8cc28acfd7a91)。
    ...
    -   **计算图像与视频的Token：**可通过以下代码计算图像或视频的 Token 消耗。估算结果仅供参考，实际用量以 API 响应为准。
        ...
        **计算图像与视频的Token**
        ...
        -   `token_pixels`：每视觉`Token`对应的像素值，不同模型情况不同：
            ...
            -   `qwen3.6系列`、`qwen3.5系列`、`Qwen3-VL`、`qwen-vl-max`、`qwen-vl-max-latest`、`qwen-vl-max-2025-08-13`、`qwen-vl-plus`、`qwen-vl-plus-latest`、`qwen-vl-plus-2025-08-15`**：**每个`Token`对应 `32x32`像素
...
-   **查看账单：**您可以在阿里云控制台的[费用与成本](https://usercenter2-intl.console.alibabacloud.com/billing#/account/overview)页面查看账单或进行充值。
    
-   **限流：**视觉理解模型的限流条件参见[限流](/help/zh/model-studio/rate-limit)。
    
-   **免费额度****（仅新加坡地域）**：从开通百炼或模型申请通过之日起计算有效期，有效期 90 天内，视觉理解模型提供 100 万 Token 的免费额度。
    

## API参考

关于视觉理解模型的输入输出参数，请参见[千问](/help/zh/model-studio/qwen-api-reference/)。
...
| --- | --- | --- | --- |
| 图像  | 大于 7MB 小于 10MB | 传入本地路径 | 仅支持公网 URL，建议使用[阿里云对象存储服务](https://www.alibabacloud.com/zh/product/object-storage-service?_p_lc=1) |
| 小于 7MB | 传入本地路径 | Base64 编码 |
| 视频  | 大于 100 MB | 仅支持公网 URL，建议使用[阿里云对象存储服务](https://www.alibabacloud.com/zh/product/object-storage-service?_p_lc=1) | 仅支持公网 URL，建议使用[阿里云对象存储服务](https://www.alibabacloud.com/zh/product/object-storage-service?_p_lc=1) |
| 大于 7MB 小于 100 MB | 传入本地路径 | 仅支持公网 URL，建议使用[阿里云对象存储服务](https://www.alibabacloud.com/zh/product/object-storage-service?_p_lc=1) |
| 小于 7MB | 传入本地路径 | Base64 编码 |
...
如果模型调用失败并返回报错信息，请参见[错误信息](/help/zh/model-studio/error-code)进行解决。
